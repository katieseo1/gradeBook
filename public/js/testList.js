...
function handleStudentDelete() {
  $('.studentTable').on('click', '.js-student-delete', function(e) {
    e.preventDefault();
    deleteStudent(
      $(e.currentTarget).closest('.js-student-delete').attr('data-id'));
  });
  }
  ...
  function editStudent(id) {
	$.ajax({
			...
			method: 'GET',
			success: function(data) {
				document.getElementById('editStudentLabel').innerHTML = "Edit Information for " + data.firstname + " " + data.lastname;
  ...
  function handleStudentEdit() {
    $('.studentTable').on('click', '.js-student-edit', function(e) {
      e.preventDefault();
      editStudent(
        $(e.currentTarget).closest('.js-student-edit').attr('data-id'));
    });
    }
...
function handleAddStudent(){
  $('#addStudentForm').submit(function(e) {
   ...
   $('#addStudent').modal('hide');
    addStudent({firstname:$(e.currentTarget).find('#firstname').val().trim(),
